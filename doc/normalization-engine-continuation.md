...
1. Aplicar `table_detection.column_aliases` e `expected_column_order` nos parsers tabulares.
2. Extrair normalizacao de valores para `normalization/amount.py`, incluindo:
   - `-R$`
   - sinal no fim
   - colunas separadas credito/debito
   - sufixos `C`/`D`
3. Criar `CanonicalTransaction` paralelo ao `NormalizedTransaction`, com:
   - banco/layout
   - pagina/linha
   - saldo
   - documento/id
   - warnings
   - confianca
4. Implementar validacao por saldo a partir de `balance_consistency_check`.
5. Transformar `examples_from_image.sample_rows` em fixtures golden sinteticas.
